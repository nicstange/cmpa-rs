...
pub fn mp_lshift_mp<T0: MPIntMutByteSlice>(op0: &mut T0, distance_len: usize) -> LimbType{
    ...
    op0.zeroize_bytes_below(distance_len.min(op0.len()));
...
fn test_mp_lshift_mp_common<T0: MPIntMutByteSlice>(op0_len: usize) {
    ...
    fn fill_limb_with_seq(first: u8, offset_len: usize) -> LimbType {
    ...
    for i in 0..op0_nlimbs + 3 {
        for j in 0..LIMB_BYTES {
            ...
            for k in 0..op0.nlimbs() {
                let val = fill_limb_with_seq(1 + (k * LIMB_BYTES) as u8, 0);
                if k != op0.nlimbs() - 1 {
            ...
            if i < op0.nlimbs() {
                let expected_val = fill_limb_with_seq(1, j);
                if i != op0.nlimbs() - 1 {
            ...
            for k in i + 1..op0.nlimbs() {
                let expected_val = fill_limb_with_seq(
